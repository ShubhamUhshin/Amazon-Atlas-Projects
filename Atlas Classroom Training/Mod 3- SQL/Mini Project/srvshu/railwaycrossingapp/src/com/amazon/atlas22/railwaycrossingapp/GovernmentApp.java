...
public class GovernmentApp {
    ...
    static ArrayList<RailwayCrossing> getCrossings(RailwayCrossingController controller,boolean rtFlag) {
        ...
        {
            /*Date now = new Date();
            LinkedHashMap<String,String> schedule = crossings.get(key).getSchedules();
            for(String from:schedule.keySet())
            {
                String to=schedule.get(from);
                DateFormat formatter = new SimpleDateFormat("hh:mm");
                try{
                    if((formatter.format(now).compareTo(formatter.format(formatter.parse(from))))>=0 && (formatter.format(now).compareTo(formatter.format(formatter.parse(from))))<=0)
                    {
                        crossings.get(key).setStatus(1);
                    }else {
                        crossings.get(key).setStatus(2);
                    }
                }catch (Exception e)
                {
